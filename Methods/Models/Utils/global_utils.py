...
class scaler(object):
    ...
    def descaleDataParallel(self, input_sequence, interaction_length):
        ...
        if self.tt == "MinMaxZeroOne":
            input_sequence = np.array(input_sequence * (self.data_max[
                                                        getFirstActiveIndex(interaction_length):getLastActiveIndex(
                                                            interaction_length)] - self.data_min[getFirstActiveIndex(
                interaction_length):getLastActiveIndex(interaction_length)]) + self.data_min[getFirstActiveIndex(
                interaction_length):getLastActiveIndex(interaction_length)])
        elif self.tt == "Standard" or self.tt == "standard":
            input_sequence = np.array(input_sequence * self.data_std[
                                                       getFirstActiveIndex(interaction_length):getLastActiveIndex(
                                                           interaction_length)].T + self.data_mean[getFirstActiveIndex(
                interaction_length):getLastActiveIndex(interaction_length)])
        elif self.tt != "no":
            raise ValueError("Scaler not implemented.")
        return input_sequence
...
def stackParallelSequenceData(sequence_data, sequence_length, prediction_length, subsample_seq,
                              parallel_group_interaction_length):
    stacked_input_data = []
    stacked_target_data = []
    ...
    for i in range(0, n - sequence_length - prediction_length, subsample_seq):
        sequence = sequence_data[i:(i + sequence_length), :]
        prediction = sequence_data[(i + sequence_length):(i + sequence_length + prediction_length),
                     getFirstActiveIndex(pgil):getLastActiveIndex(pgil)]
        stacked_input_data.append(sequence)
        stacked_target_data.append(prediction)
    return stacked_input_data, stacked_target_data
...
def getNewESNParser():
    ...
    parser.add_argument("--cell", help="cell for embedding", type=str, default='ESN')
...
def getESNParser(parser):
    ...
    parser.add_argument("--noise_level", help="noise level per mille in the training data", type=int, default=0,
                        required=True)
    parser.add_argument("--display_output", help="control the verbosity level of output , default True", type=int,
                        required=False, default=1)
    parser.add_argument("--mem_cap", help="candidate for memory capacity calculation", type=int, required=False,
                        default=0)
    parser.add_argument("--mem_cap_delay", help="mem_cap_delay", type=int, required=False, default=0)
    parser.add_argument("--learning_rate", help="learning rate for gradient descent", type=float, required=False,
                        default=1e-6)
    parser.add_argument("--number_of_epochs", help="number of epochs", type=int, required=False, default=10000)
    parser.add_argument("--solver", help="solver used to learn mapping H -> Y, it can be [pinv, saga, gd]", type=str,
                        required=False, default="pinv")
    parser.add_argument("--reference_train_time", help="The reference train time in hours", type=float, default=24)
    parser.add_argument("--buffer_train_time", help="The buffer train time to save the model in hours", type=float,
                        default=0.5)
...
def getRNNStatefullParser(parser):
    ...
    parser.add_argument("--prediction_length", help="prediction_length", type=int, required=True)
    parser.add_argument("--sequence_length", help="sequence_length", type=int, required=True)
    parser.add_argument("--hidden_state_propagation_length", help="hidden_state_propagation_length", type=int,
                        required=True)
    parser.add_argument("--scaler", help="scaler", type=str, required=True)
    parser.add_argument("--noise_level", help="noise level per mille in the training data", type=int, default=0,
                        required=True)
...
def getMLPParallelParser(parser):
    ...
    parser.add_argument("--num_parallel_groups",
                        help="groups in the output for parallelization in the spatiotemporal domain. must be divisor of the input dimension",
                        type=int, required=True)
    parser.add_argument("--parallel_group_interaction_length", help="The interaction length of each group. 0-rdim/2",
                        type=int, required=True)
    return parser
...
def createParallelTrainingData(model):
    ...
    if (not os.path.isfile(training_path_group)):
        ...
        with open(model.main_train_data_path, "rb") as file:
            ...
            dt = data["dt"]
            del data
        train_sequence_group = createParallelGroupTrainingSequence(group_num, group_start, group_end, pgil,
                                                                   train_sequence)
    ...
    if (not os.path.isfile(testing_path_group)):
        ...
        with open(model.main_test_data_path, "rb") as file:
            ...
            del data
        test_sequence_group = createParallelGroupTrainingSequence(group_num, group_start, group_end, pgil,
                                                                  test_sequence)
...
def reformatParallelGroupDataPath(model, path, gn, ll):
    ...
    last = 7
    path_ = path[:-last] + "_G{:d}-from-{:d}_GS{:d}_GIL{:d}".format(gn, model.num_parallel_groups,
                                                                    model.parallel_group_size, ll) + path[-last:]
    return path_
