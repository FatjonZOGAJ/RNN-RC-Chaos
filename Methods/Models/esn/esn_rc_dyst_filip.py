...
def set_activation_func(activ_func):
...
class esn(GlobalForecastingModel):
    ...
    def __init__(self, reservoir_size=1000, sparsity=0.01, radius=0.6, sigma_input=1, dynamics_fit_ratio=2 / 7,
                 regularization=0.0, scaler_tt='Standard', solver='auto', model_name='RC-CHAOS-ESN', activation_func='tanh', W_scaling = 1, seed=1):
        ...
        self.activation_func = set_activation_func(activation_func)
    ...
    # TODO FILIP Add revisiting echo state initialization
    def getSparseWeights(self, sizex, sizey, radius, sparsity):
        # W = np.zeros((sizex, sizey))
        # Sparse matrix with elements between 0 and 1
        # WEIGHT INIT
        W = sparse.random(sizex, sizey, density=sparsity) # TODO Make the matrix symmetric
        W *= self.W_scaling 
    ...
    def fit(self,
        ...
        for t in range(dynamics_length):
            ...
            h = self.activation_func(W_h @ h + W_in @ i)
...
def main():
    ...
    eval_all_dyn_syst(esn(reservoir_size=1000, sparsity=0.1, radius=0.5, solver='pinv',activation_func='tanh', W_scaling = 1))
